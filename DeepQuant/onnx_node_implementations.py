...
def exec_quant(x: np.ndarray, attrs: Dict[str, Any]) -> np.ndarray:
    ...
    """
    scale = np.asarray(attrs["scale"], dtype=np.float32)
    zero_point = np.asarray(attrs.get("zero_point", 0))
...
def exec_dequant(x: np.ndarray, attrs: Dict[str, Any]) -> np.ndarray:
    ...
    ``scale`` may be a per-channel array; broadcasting handles the rest.
    """
    scale = np.asarray(attrs["scale"], dtype=np.float32)
    zero_point = np.asarray(attrs.get("zero_point", 0))
...
def exec_requant_shift(
    ...
    Formula::
        ...
        out = clip(((x * mul) + add) >> log2(div), qmin, qmax)

    ``div`` is a power-of-two scalar; ``mul`` may be a per-channel vector.
    ``n_levels`` and ``signed`` determine the output clip range.
    """
    div_val = int(np.asarray(div).flat[0])
    shift = int(round(math.log2(div_val))) if div_val > 1 else 0

    # Determine output clip range from n_levels and signed (matches decompose logic)
    if bool(signed) and n_levels > 128:
        qmin, qmax = -(n_levels // 2), n_levels // 2 - 1
    elif bool(signed) and n_levels == 128:  # fused ReLU case
        qmin, qmax = 0, 127
    else:
    ...
    add64 = np.asarray(add, dtype=np.int64)

    # Broadcast per-channel mul and add across spatial dims
    if mul64.ndim > 0 and mul64.size > 1:
        shape = [1] * x64.ndim
        shape[1] = mul64.size  # channel axis = 1
        mul64 = mul64.reshape(shape)

    if add64.ndim > 0 and add64.size > 1:
        shape = [1] * x64.ndim
        shape[1] = add64.size  # channel axis = 1
        add64 = add64.reshape(shape)

    out = ((x64 * mul64) + add64) >> shift
    return np.clip(out, qmin, qmax).astype(np.int8 if bool(signed) else np.uint8)
...
def run_onnx_graph(
    ...
    * MeZO perturbation ops – PerturbNormal, PerturbUniform, PerturbTriangle,
    ...
    Args:
        onnx_path:   Path to the ONNX model file, or an ``onnx.ModelProto``.
    ...
    """
    if isinstance(onnx_path, onnx.ModelProto):
        model = onnx_path
    else:
    ...
    for node in graph.node:
        ...
        # ------------------------------------------------------------------
        if op == "Quant":
            # scale/zero_point may be attributes OR initializer inputs (possibly per-channel)
            merged = dict(attrs)
            if "scale" not in merged and len(node_inputs) > 1 and node_inputs[1] is not None:
                merged["scale"] = np.asarray(node_inputs[1])
            if "zero_point" not in merged and len(node_inputs) > 2 and node_inputs[2] is not None:
                merged["zero_point"] = np.asarray(node_inputs[2])
            out = exec_quant(node_inputs[0], merged)
            ...
            if "zero_point" not in merged and len(node_inputs) > 2 and node_inputs[2] is not None:
                merged["zero_point"] = np.asarray(node_inputs[2])
            out = exec_dequant(node_inputs[0], merged)

        elif op == "RequantShift":
            x, mul, add = node_inputs
            out = exec_requant_shift(
                x, mul, add, attrs["div"],
                n_levels=int(attrs.get("n_levels", 256)),
                signed=int(attrs.get("signed", 1)),
            )
